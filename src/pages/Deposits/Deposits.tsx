...
import { UpdateDeposti } from './UpdateDeposit';
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await axios.get(
        'https://api.ajgameshop.xyz/api/v1/deposit/admin',
        {
          headers: {
            Authorization: `${token}`,
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `https://api.ajgameshop.xyz/api/v1/deposit/${id}`,
            {
              headers: {
                Authorization: token,
                'Content-Type': 'application/json',
              ...
              {filteredData
                ...
                ?.map((deposti: IDeposit, key: number) => (
                  <tr key={key}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">
                        {deposti?.userId?.email}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">
                        {deposti?.userId?.phone}
                      </p>
                    </td>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">
                        {formatToLocalDate(deposti.createdAt)}
                      </p>
                    </td>
                    <td className="border-b border-[#eee] py-5 px-3 dark:border-strokedark">
                      <div className="flex items-center space-x-3.5">
                        ...
                        {/* edit btn */}
                        <button
                          onClick={() => openModal(deposti)}
                          className={` ${
                            deposti.status == 'complete'
                              ? ''
                              : 'hover:text-primary'
                          }`}
                          disabled={deposti.status === 'complete'}
                        >
                          <svg
